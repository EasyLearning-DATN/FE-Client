...
export class CreateLessonComponent implements OnInit {
  ...
  const idIndex = segments.indexOf('classroom') + 1; // Chỉ số của segment ID
  if (idIndex !== 0 && idIndex < segments.length) {
    this.idClass = segments[idIndex];
    console.log('ID từ URL:', this.idClass);
    // Bạn có thể thực hiện các xử lý khác với id ở đây
  }
    this.getAllLesson();
  }
